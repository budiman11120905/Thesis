...
while ($data = mysql_fetch_array($hasil))
{
  ...
  // jika kata terdepan dari SMS adalah 'INFO' maka cari Info SMS
  if ($pecah[0] == "INFO")
  {
     // baca WEBMAKER dari pesan SMS
     $nim = $pecah[1];

     // cari INFO berdasar WEBMAKER
     $query2 = "SELECT info FROM infosms WHERE perintah = '$nim'";
     $hasil2 = mysql_query($query2);

     // cek bila data nilai tidak ditemukan
     if (mysql_num_rows($hasil2) == 0) $reply = "Informasi terbaru tidak ditemukan";
     else
     {
        // bila nilai ditemukan
        $data2 = mysql_fetch_array($hasil2);
        $nilai = $data2['info'];
        $reply = "Informasi Saat ini untuk : ".$nilai;
     }
  }
  else $reply = "Maaf perintah yang anda masukkan salah";

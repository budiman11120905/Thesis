...
while ($data = mysql_fetch_array($hasil))
{
  ...
  // jika kata terdepan dari SMS adalah 'INFO' maka cari Info webmaker applaunch
  if ($pecah[0] == "INFO")
  {
     ...
     $query2 = "SELECT info FROM webmakerapplaunch WHERE webmaker = '$nim'";
     $hasil2 = mysql_query($query2);

     // cek bila data nilai tidak ditemukan
     if (mysql_num_rows($hasil2) == 0) $reply = "Informasi webmaker tidak ditemukan";
     else
     {
        // bila nilai ditemukan
        $data2 = mysql_fetch_array($hasil2);
        $nilai = $data2['info'];
        $reply = "Informasi Saat ini untuk webmaker applaunch: ".$nilai;
     }
  }
